              ...
              ... on RouteInternal {
                entity {
                  ...
                  ... on NodeHomepage {
                    ...
                    featuredProjectsTitle ctaTitle ctaDescription { processed } ctaPrimary ctaSecondary
                  }
                  ... on NodeProject {
                    __typename id title path body { processed }
                    projectArea { ... on TermInterface { id name } }
                    startDate { timestamp } location fundingGoal
                  ...
                  ... on NodeEvent {
                    __typename id title path body { processed }
                    eventDate { timestamp } endDate { timestamp } location
                    eventType { ... on TermInterface { id name } } registrationUrl
                    image { url alt width height variations(styles: [LARGE, MEDIUM]) { name url width height } }
                  }
                  ... on NodeNews {
                    __typename id title path created { timestamp } body { processed }
                    category { ... on TermInterface { id name } } featured
                    image { url alt width height variations(styles: [LARGE, MEDIUM]) { name url width height } }
                  }
                }
              }
